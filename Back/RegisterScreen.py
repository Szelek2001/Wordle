...
class RegisterScreen(tk.Frame):
    ...
    def init_ui(self):
        ...
        tk.Label(container, text="Rejestracja", fg="#f0f3f5", bg=text_file.config.COLOR_BACKGROUND,
        ...
        tk.Button(
            container, text="Zarejestruj  się", fg=text_file.config.Color_FONT, bg="#1d1d1f", border=0, cursor="hand2",
    ...
    def check(self):
        dao = DaoWordle()
        if dao.check_usr(self.login.get()):
            self.toast("Jest już użytkownik o takim nicku!", 3)
            return False
